...
for (m in mpalist) {
  ...
  r=1
  for (i in 1:length(r)) {
    Pixel_in<-subset_in[1,]
    Pixel_in_rand<-subset_in[sample(nrow(subset_in),1),]
    Pixel_out_rand<-subset_out[sample(nrow(subset_out),2),]

    
    comparison_id<-paste(Pixel_A[,41],Pixel_B[,41],sep="-");comparison_id
    Pixel_A_id<-Pixel_A[,3]
    Pixel_B_id<-Pixel_B[,3]
    relative_protection<-Pixel_A[,4]-Pixel_B[,4];relative_protection
    year=names(subset[c(11:38)])
    Kelp_A<-t(Pixel_A[c(11:38)]);colnames(Kelp_A)<-"kelp_A"
    Kelp_B<-t(Pixel_B[c(11:38)]);colnames(Kelp_B)<-"kelp_B"
    coordinates(Pixel_A)<-~X+Y
    coordinates(Pixel_B)<-~X+Y
    pixel_dist<-gDistance(Pixel_A,Pixel_B)
    Out_comparison<-cbind.data.frame(name,comparison_id,Pixel_A_id,Pixel_B_id,relative_protection,
                               pixel_dist,year,Kelp_A,Kelp_B)
    Out_comparison$Delta<-Out_comparison$kelp_A-Out_comparison$kelp_B
    Out_comparison_DF<-rbind.data.frame(Out_comparison_DF,Out_comparison)
  }
}
...
InIn<-subset(Out_comparison_DF,Out_comparison_DF$comparison_id=="1-1")
OutOut<-subset(Out_comparison_DF,Out_comparison_DF$comparison_id=="0-0")
InOut<-subset(Out_comparison_DF,Out_comparison_DF$comparison_id=="1-0")
OutIn<-subset(Out_comparison_DF,Out_comparison_DF$comparison_id=="0-1")
...
for (m in mpalist){
  ...
  InOut_sub<-subset(InOut,InOut$name==m)
  OutIn_sub<-subset(OutIn,OutIn$name==m)
  OutOut_sub<-subset(OutOut,OutOut$name==m);OutOut_sub$YEAR<-gsub("X","",OutOut_sub$year)
  mpa=get(m)
  if (nrow(InIn_sub)>0 & nrow(InOut)>0 & nrow(OutIn)>0 & nrow(OutOut)>0){
    OutIn_sub$comparison_id<-"1-0";OutIn_sub$Delta<-OutIn_sub$Delta*-1
    InOut_sub<-c(InOut_sub,OutIn_sub);InOut_sub$YEAR<-gsub("X","",InOut_sub$year)
    ##Averaging deltas by year
    InOut_sub_agg<-aggregate(InOut_sub$Delta,by=list(InOut_sub$year),mean,na.rm=TRUE);InOut_sub_agg$YEAR<-gsub("X","",InOut_sub_agg$Group.1);InOut_sub_agg$comparison<-c("InOut")
    OutOut_sub_agg<-aggregate(OutOut_sub$Delta,by=list(OutOut_sub$year),mean,na.rm=TRUE);OutOut_sub_agg$YEAR<-gsub("X","",OutOut_sub_agg$Group.1);OutOut_sub_agg$comparison<-c("OutOut")
    InIn_sub_agg<-aggregate(InIn_sub$Delta,by=list(InIn_sub$year),mean,na.rm=TRUE);InIn_sub_agg$YEAR<-gsub("X","",InIn_sub_agg$Group.1);InIn_sub_agg$comparison<-c("InIn")
