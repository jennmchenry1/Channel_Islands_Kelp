  ...
  list2<-do.call("rbind",lapply(iterator,function(j) {
    ...
    if (sum(r)>0) {
      list3<-do.call("rbind",lapply(c(1:sub_len),function (k){
        ...
        PixelCompare(Pixel_in_rand,Pixel_out_rand)
        # paste(i,j,name,sub_len,k,"yes",sep="_")
      }))
      return(list3)
    }
      write.csv(list3,paste("Pixel_Comparison_Output_Iteration_",j,".csv",sep=""))
  }))
  return(list2)
}
stopCluster(cl)
...
temp = list.files(pattern="Pixel_Comparison_Output_Iteration*")
