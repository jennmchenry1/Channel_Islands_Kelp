...
mpalist<-as.list(unique(FINAL$name));mpalist
...
data_2<-data.frame(NULL)
data_3<-data.frame(NULL)

Parloop<-function(FINAL){
  mpalist<-unique(FINAL$name)
  data_output<-do.call("rbind",lapply(c(1:12),function(x){
    m=mpalist[x]
    ...
    if (sum(r)==0) {
      print("No inside pixels!")
    }
    if (sum(r)>0) {
      data<-do.call("rbind",lapply(c(1:r),function (xx){
        print(subset_in[1,2])
        #Grabing inin, in out, and out out comparison pixels
        Pixel_in_rand<-as.data.frame(rbind(subset_in[xx,],subset_in[sample(nrow(subset_in),1),]))
        Pixel_out_rand<-as.data.frame(subset_out[sample(nrow(subset_out),2),])
        Compare_MPAs<-PixelCompare(Pixel_in_rand,Pixel_out_rand);print(Compare_MPAs[1,1])
        return(Compare_MPAs)
        }))
      # data_2<-rbind(data_2,as.data.frame(data))
      return(data)
    }
  }))
  return(data_output)
  write.csv(data_output,file=paste("Compare_ALL_MPAS_",m,".csv",sep=""))
}
...
mclapply(c(1:2),function (X){
  data_output_mc<-Parloop(FINAL)
  write.csv(data_output_mc,file="Compare_ALL_MPAs_version_",X,".csv",sep="")
})
